...
const ConnDialog = ({ artist, related, id }) => {
  ...
  useEffect(() => {
    axios.get(`https://us-central1-cotton-eyed-joe.cloudfunctions.net/widgets/relationships?id=${id}`).then(res => {
  ...
  const renderCover = (i) => {
    ...
    return (
      ...
      <Grid item container direction='row' justify='center' alignItems='center' spacing={3} className={classes.title}>
        ...
        <Grid item>
          <Chip key={data.cover[i].type} label={MUSIC_RELATIONSHIPS[data.cover[i].type]} />
        </Grid>
  ...
  const renderRef = (i) => {
    ...
    return (
      ...
      <Grid item container direction='row' justify='center' alignItems='center' spacing={3} className={classes.title}>
        <Grid item>{related}</Grid>
        <Grid item>
          <Chip key={data.ref[i].type} label={MUSIC_RELATIONSHIPS[data.ref[i].type]} />
        </Grid>
        <Grid item>{artist}</Grid>
      </Grid>
